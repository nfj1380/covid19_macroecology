...
mod <- "all" # "sub" or "all"
if(mod == "sub"){
  if(resp == "cases") vars_end <- vars_endemic[!vars_endemic %in% c("Ascariasis", "Malaria","HIV.AIDS")] # covariates to drop
  if(resp == "deaths") vars_end <- vars_endemic[!vars_endemic %in% c("Ascariasis", "Malaria","HIV.AIDS")] # covariates to drop
} else vars_end <- vars_endemic
if(resp == "deaths") vars_num <- c(vars_numeric,"log_cases") else vars_num <- vars_numeric

# fit models ----
if(F){
  
  f_mu <- mu_spline(resp, vars_num, vars_factor, vars_end)
