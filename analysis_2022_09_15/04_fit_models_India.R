...
cvd19_india <- read_csv("data/India_covid19_2021_08_30.csv") %>% 
  rename(cases = Cases_1July2021_cumulative, 
         ...
         tests = Tested_1July2021_cumulative,
         pop=Population ) %>% 
...
cvd19_imp <- 
  ...
  map(~ .x %>%# transform variables
        mutate(log_tests_pp= log(tests)) %>% 
        mutate(across(where(is.character), as.factor)) %>% 
        mutate(log_healthcare = log(Health_expenditure),
               log_cases = log(cases), log_pop = log(pop))) %>% 
...
#Trich prev not corr with ascariasis in India
  vars_endemic <- c( "Ascariasis","Malaria","HIV","Trichuriasis", "Tubercluosis", 'Hookworm'); vars_endemic
  vars_numeric <- cvd19_imp[[1]] %>% 
    select(where(is.numeric),  -pop, -Lat , -Long,
           -Health_expenditure, -Density, #keep pop in? health strongly cor with pop. Density/urban strongly corr
           -contains(c("health", "cases","death",'tests')), -all_of(vars_endemic)) %>% 
  names; vars_numeric
...
if (mod=='lm') f_mu <- mu_linear(resp, offset, vars_numeric, vars_endemic)
f_shape <- shape ~ 1
...
if(T){
  ...
  #individual model
  brm(f_model, 
      data = cvd19_imp[[1]], #first imputed data set
      cores = cores, 
      chains = chains,
      init_r = init_r, 
      iter = iter, 
...
fit_cases <- brm_multiple(f_model, 
...
if(F){
  ...
  fit_cases <- readRDS(paste0("results/fitIndia_",mod,"_",resp,"_",run_date,".rds"))
  ...
  ce_plots <- pbmclapply(vars_plot, function(X) plot_ce(fit_cases,X,resp = "cases",exp = ifelse(stringr::str_starts(X,"log"),T,F)), 
...
if(F){
  ...
  pop_set <- cvd19_imp[[1]]$pop # cvd19$pop or 1e6 etc.
  pp_raw <- posterior_epred(fit_, newdata = cvd19_imp[[1]] %>% 
                              mutate(log_tests_pp = log((exp(log_tests_pp)/pop)*pop_set), 
                                     log_pop = log(pop_set))) %>% 
    as.data.frame()
  
  colnames(pp_raw) <- cvd19_imp[[1]]$Province_State
  pp_raw %>% 
    as_tibble() %>% 
