...
vars_numeric <- cvd19 %>% select(where(is.numeric), -pop, -cases, -spatLag, -contains(c("death")), -all_of(vars_endemic)) %>% 
  names; vars_numeric
...
if(F){
  ...
  resp <- "deaths"
  theme_set(theme_classic())
  fit <- readRDS(paste0("results/fit_",mod,"_",resp,"_2022_07_21.rds"))
  
  # check convergence (Rhat<1.1): very good
  rhat(fit) %>% max # 1.007

  ce_plots <- pbmclapply(vars_endemic, function(X) plot_ce(fit,X,resp = resp,exp = ifelse(str_starts(X,fixed("log_")),T,F)), mc.cores = 12)
  ...
  ggsave(paste0("plots/ce_plots_",resp,"_",mod,"_vars_endemic_2022_07_21.pdf"), width = 9, height = 9, device = cairo_pdf)
...
if(F){
  ...
  pp_raw %>% 
    ...
    labs(title = "Global deaths", 
         ...
         x = "Countries") #  per 1,000,000 people
  
  #ggsave("plots/global_deaths_ppd_2022_07_21.pdf", height = 6, width = 8)
